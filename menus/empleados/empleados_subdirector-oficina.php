...
session_start();
	...
	if($c==null){
		...
		if($_SESSION['id']=='33'){
			$sql="SELECT nombre, password, id, rango, login, zona, oficina, descripcion,image, cartera  FROM usuarios WHERE jefe='subdirector' AND oficina='$oficina' AND cartera = '$cartera'";

		}
		else{
			$sql="SELECT nombre, password, id, rango, login, zona, oficina, descripcion,image, cartera  FROM usuarios WHERE jefe='subdirector' AND oficina='$oficina'";
		}
