...
session_start();
	...
	if($_SESSION['id']=='25'){
		$oficina = 'galicia norte';
		$jefe = 'inspectorjavier';
	}
	if($_SESSION['id']=='65'){
		$oficina = 'ponferrada';
		$jefe = 'inspectorjosema';
	}
	...
	if($c==null){
		...
		if($_SESSION['id']=='23'){
			$sql="SELECT nombre, password, id, rango, login, zona, oficina, descripcion,image, cartera  FROM usuarios WHERE jefe='$jefe' AND oficina='$oficina'";
		}
		elseif($_SESSION['id']=='25'){
			$sql="SELECT nombre, password, id, rango, login, zona, oficina, descripcion,image, cartera  FROM usuarios WHERE jefe='$jefe' AND oficina='$oficina'";
		}
		elseif($_SESSION['id']=='65'){
			$sql="SELECT nombre, password, id, rango, login, zona, oficina, descripcion,image, cartera  FROM usuarios WHERE jefe='$jefe' AND oficina='$oficina'";
		}

		else{
			$sql="SELECT nombre, password, id, rango, login, zona, oficina, descripcion,image, cartera  FROM usuarios WHERE jefe='inspector' AND oficina='$oficina'";
