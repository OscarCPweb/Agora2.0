...
<?php
	...
	if($c==null){
		...
		if($_SESSION['id']=='15'){
			$sql="SELECT nombre, password, id, rango, login, zona, oficina, descripcion,image, cartera  FROM usuarios WHERE jefe='director oficina' AND oficina='cadiz'";
		}
		else{
			$sql="SELECT nombre, password, id, rango, login,zona, oficina,descripcion,image, cartera
			FROM usuarios WHERE jefe='director territorial' AND zona= '$zona'";		}
